namespace :db do
  ...
  task :bootstrap  => :environment do
    ...
    Dir.glob(File.join(PromotionsExtension.root, "db", 'sample', '*.{yml,csv}')).each do |fixture_file|
      Fixtures.create_fixtures("#{PromotionsExtension.root}/db/sample", File.basename(fixture_file, '.*'))
    end
  end
end
