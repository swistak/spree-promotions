(function($) {
    ...
    var AutoCompleteLocal = function(element, update, data_source, options) {
    ...
    $.extend(AutoCompleteLocal.prototype, {
        show: function() {
        ...
        getUpdatedChoices: function() {
            ...
            var data = $.grep(this.data_source(), function(s){ return(s.indexOf(token) >= 0)});
            var formated_data = '<ul>'+$.map(data, function(s){ return('<li>'+s+'</li>') } ).join("")+'</ul>'
            this.updateChoices(formated_data);
        }
    });

    $.fn.autocomplete_local = function(options) {
       var update = $('#' + options.update).get(0);
       var data_source = options.data_source;
       if (typeof(update) == "undefined") {
         update = $(document.createElement('div')).
                             attr('class', 'auto_complete').
                             attr('id', this.id + '_auto_complete').
                             attr('style', 'display:none');
         this.after(update);
         update = update.get(0);
       }

       options.update = options.data_source = undefined;

       this.filter(':text').each(function() {
         new AutoCompleteLocal(this, update, data_source, options);
       });
